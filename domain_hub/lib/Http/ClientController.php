...
// 设置页面标题和面包屑
                            ...
                            if (in_array($action, ['ajax_initiate_domain_gift','ajax_accept_domain_gift','ajax_cancel_domain_gift','ajax_list_domain_gifts'], true)) {
                                ...
                                try {
                                    ...
                                    if ($action === 'ajax_initiate_domain_gift') {
                                        ...
                                        $result = Capsule::transaction(function () use ($subdomainId, $userId, $expiresAt, $settingsForGift, $nowStr) {
    ...
    private function resolveAnnouncementState(array $settings): array
    {
        return [
            'enabled' => !empty($settings['client_notice_enabled']),
            'message' => trim((string)($settings['client_notice'] ?? '')),
        ];
    }
